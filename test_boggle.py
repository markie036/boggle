...
class TestBoggle(unittest.TestCase):
    ...
    def test_grid_coordinates(self):
        ...
        self.assertIn((0, 0), grid)#assertIn method used to check that 0, 0 is in a 2 x 2 grid
        self.assertIn((0, 1), grid)
        self.assertIn((1, 0), grid)
        self.assertIn((1, 1), grid)
        self.assertNotIn((2, 2), grid)#assertNotIn used to check that 2, 2 is not in a 2 x 2 grid
    ...
    def test_converting_a_path_to_a_word(self):
        """
        Ensure that the paths can be converted to words
        """
        grid = boggle.make_grid(2, 2)
